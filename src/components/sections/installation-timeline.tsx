...
export default function InstallationTimeline() {
  const steps = [
    {
      icon: <MapPin className="w-5 h-5 text-dark" />,
      title: "Site Survey",
      duration: "Day 1",
    ...
    {
      icon: <Hammer className="w-5 h-5 text-dark" />,
    ...
    {
      icon: <Cable className="w-5 h-5 text-white" />,
  ...
  return (
    <section id="installation" className="py-20 bg-slate-900 text-white border-t border-white/5 relative">
      <div className="container mx-auto px-6 max-w-7xl">
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <p className="text-slate-300 font-sans text-sm md:text-base">
            A transparent, step-by-step process with zero guesswork — we handle everything from paperwork to power-on in 21 days.
          </p>
        </div>
        ...
        <div className="hidden lg:block relative">
          {/* Connector */}
          <div className="absolute top-10 left-16 right-16 h-0.5 bg-white/10 z-0" />

          <div className="grid grid-cols-6 gap-4 relative z-10">
            {steps.map((st, idx) => (
              <div key={idx} className="flex flex-col items-center text-center">
                {/* Icon bubble */}
                <div className={`w-20 h-20 rounded-full ${st.color} flex items-center justify-center shadow-lg mb-4 ring-4 ring-slate-900`}>
                  {st.icon}
                </div>
                <span className="font-numbers text-xs font-bold text-primary mb-1">{st.duration}</span>
                <h4 className="font-extrabold text-sm text-white mb-2">{st.title}</h4>
        ...
        <div className="lg:hidden space-y-4">
          {steps.map((st, idx) => (
            <Card key={idx} variant="glass-dark" className="p-5 border border-white/10">
              <div className="flex gap-4 items-start">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs font-numbers font-bold text-primary bg-white/10 px-2 py-0.5 rounded-full">{st.duration}</span>
                  </div>
                  <p className="text-xs text-slate-400 leading-relaxed font-sans">{st.desc}</p>
                </div>
              </div>
